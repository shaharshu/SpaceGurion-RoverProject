...
#include "../../src/RoverProject/Stepper/SmartStepperController.h"
...
void loop() {
  ...
  mySmartStepper.MoveSteps(STEPS_PER_REV);   // 360° one way
  delay(1000);
  
  Serial.println("Half revolution clockwise");
  mySmartStepper.MoveSteps(STEPS_PER_REV/2);   // 180 one way
  delay(1000);

  Serial.println("One revolution counterclockwise");
  mySmartStepper.MoveSteps(-STEPS_PER_REV);  // 360° the other way
  delay(1000);
}
